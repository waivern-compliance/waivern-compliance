...
Extend existing `ArtifactStore` with:

```python
class ArtifactStore(ABC):
    # Existing
    def save(step_id, message) -> None: ...
    def get(step_id) -> Message: ...

    # New
    def save_raw(artifact_id, data: bytes, content_type) -> None: ...
    def get_raw(artifact_id) -> tuple[bytes, str]: ...
    def list_artifacts(prefix) -> list[str]: ...
```
