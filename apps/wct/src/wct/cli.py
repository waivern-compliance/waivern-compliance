...
from waivern_llm import BatchResultPoller, LLMService, LLMServiceFactory, PollResult
...
def poll_run_command(run_id: str, log_level: str = "INFO") -> None:
    ...
    try:
        ...
        error_panel = Panel(
            f"[red]{e}[/red]", title="❌ Poll failed", border_style="red"
        )
        console.print(error_panel)
        raise typer.Exit(1) from e
...
def _format_poll_result(result: PollResult, run_id: str, metadata: RunMetadata) -> None:
    ...
    if result.pending == 0 and result.completed > 0 and result.failed == 0:
