"""Result builder for GDPR service integration classification output.
...
class GDPRServiceIntegrationResultBuilder:
    """Builds output results for GDPR service integration classification.
    ...
    def build_output_message(
        self,
        findings: list[GDPRServiceIntegrationFindingModel],
        ...
        Args:
            findings: Classified GDPR service integration findings.
        ...
        output = GDPRServiceIntegrationFindingOutput(
            findings=findings,
            summary=summary,
        ...
        output_message = Message(
            id="gdpr_service_integration_classification",
            content=result_data,
        ...
        logger.info(
            f"GDPRServiceIntegrationClassifier processed with {len(result_data['findings'])} findings"
        )

        return output_message

    def _build_summary(
        self, findings: list[GDPRServiceIntegrationFindingModel]
    ) -> GDPRServiceIntegrationSummary:
        ...
        """
        # Count by GDPR purpose category
        category_counts = Counter(f.gdpr_purpose_category for f in findings)
        ...
        return GDPRServiceIntegrationSummary(
            total_findings=len(findings),
            gdpr_purpose_categories=dict(sorted(category_counts.items())),
