...
class DataCollectionAnalyser(Analyser):
    """Analyser for detecting data collection mechanisms in source code.

    Uses predefined rulesets to identify data collection patterns and
    assign collection_type and data_source to each match. No LLM
    ...
    def __init__(self, config: DataCollectionAnalyserConfig) -> None:
        ...
        rules = RulesetManager.get_rules(
            config.pattern_matching.ruleset, DataCollectionRule
        )
        self._handler = SourceCodeSchemaInputHandler(
            rules=rules,
            context_window=config.source_code_context_window,
        )
        self._result_builder = DataCollectionResultBuilder(config)
    ...
    def _load_reader(self, schema: Schema) -> SchemaReader[SourceCodeDataModel]:
        ...
        return importlib.import_module(  # type: ignore[return-value]
            f"waivern_data_collection_analyser.schema_readers.{module_name}"
        )
