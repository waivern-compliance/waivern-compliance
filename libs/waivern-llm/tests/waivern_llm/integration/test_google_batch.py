"""Integration tests for Google Gemini batch API operations.

These tests make real API calls to Google's Gemini Batch API and may take
...
@pytest.mark.batch
class TestGoogleBatchIntegration:
    """End-to-end tests for Google Gemini BatchLLMProvider methods."""

    async def test_submit_poll_and_retrieve_batch_results(
        self, require_google_api_key: str
    ) -> None:
        """Full flow: submit batch, poll until completed, retrieve and verify results."""
        provider = GoogleProvider(
            api_key=require_google_api_key, model="gemini-2.5-flash"
        )
    ...
    async def test_cancel_batch(self, require_google_api_key: str) -> None:
